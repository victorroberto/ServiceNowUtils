function ($scope, spUtil, spUIActionsExecuter) {
	...
	c.approver = function (action, sys_id, comment, number) {
		c.loading = sys_id
		c.server.get({
			action: action,
			sys_id: sys_id,
	...
	c.data.counter = c.filteredApprovals.length || 0;
	c.data.query = ""
	c.filter = function () {
	...
	c.showMore = function () {
		...
		for (var i = c.data.currentItems; i < c.data.currentItems + 5; i++) {
