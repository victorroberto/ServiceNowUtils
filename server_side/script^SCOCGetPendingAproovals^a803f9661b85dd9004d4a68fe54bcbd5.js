...
SC_OC_GetPendingAproovals.prototype = Object.extendsObject(AbstractAjaxProcessor, {
    ...
    getAprovacoes: function (sistema, userInput) {
        ...
        registroLog = this._insereLog('criarRegistroLog', '', user, '', '');
        ...
        if (!this._permiteExecucao()) {
            ...
            this._insereLog('atualizaLog', registroLog, '', '', "Outra execução já está em andamento.");
            this._insereLog('atualizaDescricao', registroLog, '', '', "Outra execução já está em andamento");
        ...
        if (!obj_user.hasRole('approver_user')) {
            /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
            this._insereLog('atualizaLog', registroLog, '', '', "O usuário não tem permissão de Aprovador");
            this._insereLog('atualizaDescricao', registroLog, '', '', "O usuário não tem permissão de Aprovador");
            this._insereLog('encerraLog', registroLog, '', '', '');
        ...
        if (!user) {
            /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
            this._insereLog('atualizaLog', registroLog, '', '', "Usuário não encontrado");
            this._insereLog('atualizaDescricao', registroLog, '', '', "Usuário não encontrado");
            this._insereLog('encerraLog', registroLog, '', '', '');
        ...
        switch (sistema) {
            case 'sapiens':
                ...
                getAprovacoesSAPIENS.montaPayloadAprovacoes(user, obj_user, registroLog);
                break;
            case 'mv':
    ...
    _apagaaprovacoesAdm: function () {
        var grSysApprover = new GlideRecord('sysapproval_approver');
        grSysApprover.addEncodedQuery('approver=6816f79cc0a8016401c5a33be04be441^stateINrequested,approved^source_table=u_item_da_solicitacao_compra^ORsource_table=u_ordem_compra^ORsource_table=u_solicitacao_compra');
        grSysApprover.query();
        grSysApprover.deleteMultiple();
    },
    ...
    _insereLog: function (tipo, registroLog, user, sistema, worklog) {
        ...
        if (tipo == 'atualizaLog') { //('atualizaLog', registroLog, '', '', worklog);
            var grLog = new GlideRecord('u_scoc_controle');
            grLog.addQuery('sys_id', registroLog);
            grLog.query();
            if (grLog.next()) {
        ...
        if (tipo == 'encerraLog') { //('encerraLog', registroLog, '', '', '')
            var grLog = new GlideRecord('u_scoc_controle');
            grLog.addQuery('sys_id', registroLog);
            grLog.query();
            if (grLog.next()) {
        ...
        if (tipo == 'atualizaDescricao') { //('atualizaDescricao', registroLog, '', '', '')
            var grLog = new GlideRecord('u_scoc_controle');
            grLog.addQuery('sys_id', registroLog);
            grLog.query();
            if (grLog.next()) {
