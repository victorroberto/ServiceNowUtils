/* Script responsavel por criar a solicitação de compra e seus itens ou a ordem de compra e seus itens do sistema MV2000 */

var SC_OC_MV_Sync = Class.create();
SC_OC_MV_Sync.prototype = {
    ...
    syncSc: function (integrador_gr, registroLog) {
        ...
        if (integrador_gr.getValue('u_data') == null) {
            ...
            this._insereLog('atualizaWorklog', registroLog, 'Solicitações de Compra - MV2000: ' + integrador_gr.getValue('u_data'), '', '');
        }

        /* Adiciona dados recebidos na tabela da solicitação de compras */
        function persisteSC(grSolicitacaoCompra, sc) {
            grSolicitacaoCompra.setValue("u_codigo_da_filial", sc.cd_multi_empresa + ' - ');
            grSolicitacaoCompra.setValue("u_numero_da_solicitacao", sc.cd_sol_com);
            grSolicitacaoCompra.setValue("u_sistema_de_origem", "MV2000_WS");
            ...
            if (sc.tp_sol_com == "S") {
                grSolicitacaoCompra.setValue("u_solicitacao_de_produto_ou_servico", "SERVIÇO");
            } else if (sc.tp_sol_com == "P") {
                grSolicitacaoCompra.setValue("u_solicitacao_de_produto_ou_servico", "PRODUTO");
            }

            if (sc.nm_solicitante && sc.nm_solicitante !== null && sc.nm_solicitante !== "null") {
        ...
        function persisteSCItem(grItem, item, order_id, numero) {
            ...
            if (item.TP_SOL_COM == "S") {
                ...
                grItem.setValue("u_sistema_de_origem", "MV2000_WS");
                grItem.setValue("u_tipo_solicitacao_compra", "SERVIÇO");
                grItem.setValue("u_codigo_do_servico", item.CD_SERVICO);
            } else {
                if (item.DS_PRODUTO && item.DS_PRODUTO !== null && item.DS_PRODUTO !== 'null' && item.DS_PRODUTO !== undefined && item.DS_PRODUTO !== 'undefined') {
        ...
        function persisteSCListaUltimaCompra(item, sysId_item) {
            ...
            item.listaUltimaCompra.forEach(function (itemHistoricoCompra) {
                ...
                grHistoricoProduto.setValue('u_cd_sequencia', itemHistoricoCompra.CD_SEQUENCIA);
                grHistoricoProduto.setValue('u_numero_da_solicitacao', itemHistoricoCompra.CD_TIPO);
                grHistoricoProduto.setValue('u_tipo', itemHistoricoCompra.TIPO);
                grHistoricoProduto.setValue('u_descricao', itemHistoricoCompra.DS_PRODUTO);
                grHistoricoProduto.setValue('u_qtd_entrada', itemHistoricoCompra.QT_ENTRADA);
                grHistoricoProduto.setValue('u_vl_custo_real', itemHistoricoCompra.VL_CUSTO_REAL);
                grHistoricoProduto.setValue('u_dt_entrada', itemHistoricoCompra.DT_ENTRADA);
                grHistoricoProduto.setValue('u_cd_fornecedor', itemHistoricoCompra.CD_FORNECEDOR);
                grHistoricoProduto.setValue('u_nome_do_fornecedor', itemHistoricoCompra.NM_FORNECEDOR);
        ...
        /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
        this._insereLog('insereQtdSolicitacoes', registroLog, '', '', solicitacao.length);
        ...
        for (var i = 0; i < solicitacao.length; i++) {
            ...
            if (solicitacao_id) {
                ...
                sc.itens.map(function (item) {
                    ...
                    sysId_item = persisteSCItem(grItemUpdate, item, solicitacao_id, sc.cd_sol_com);
                ...
                this.addApprover(integrador_gr.getValue("u_approver"), grSolicitacaoCompra, "requested");
            } else {
                gs.log('Erro ao criar SC = ' + JSON.stringify(sc), 'SC_OC_MV_Sync');
            }
            ...
            /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
            this._insereLog('atualizaWorklog', registroLog, 'Solicitação ' + sc.cd_sol_com + ' processada.', '', '');
        }
        /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
    ...
    syncOc: function (integrador_gr, registroLog) {
        ...
        if (integrador_gr.getValue('u_data') == null) {
            ...
            this._insereLog('atualizaWorklog', registroLog, 'Ordens de Compra - MV2000: ' + integrador_gr.getValue('u_data'), '', '');
        }
        ...
        function persisteOC(grOrdemCompra, oc) {
            ...
            grOrdemCompra.setValue("u_codigo_da_empresa", oc.cd_multi_empresa + ' - ');
            grOrdemCompra.setValue("u_numero_da_ordem", oc.cd_ord_com);
            grOrdemCompra.setValue("u_sistema_de_origem", "MV2000_WS");
            grOrdemCompra.setValue("u_data_de_emissao", oc.dt_ord_com);
            grOrdemCompra.setValue("u_condicao_de_pagamento", oc.ds_condicao_pagamento);
            ...
            if (oc.nm_solicitante && oc.nm_solicitante !== null && oc.nm_solicitante !== "null") {
        ...
        for (var i = 0; i < ordemCompra.length; i++) {
            ...
            if (order_id) {
                ...
                oc.itens.map(function (item) {
                    grItem.initialize();
                    sysId_item = persisteOCItem(grItem, item, order_id, oc.cd_ord_com, oc.tp_ord_com);
                ...
                gs.log('Erro ao criar OC = ' + JSON.stringify(oc), 'SC_OC_MV_Sync');
            }
            ...
            /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
            this._insereLog('atualizaWorklog', registroLog, 'Ordem ' + oc.cd_ord_com + ' processada.', '', '');
        }
        /* Atualiza registro de controle na tabela SCOC Controle. Esse registro serve para acompanhamento do status da integração. */
        ...
        if (nextApprover != 'NULL') {
            ...
            if (grUser.get('user_name', nextApprover)) {
                ...
                if (approval_check_gr) {
                    ...
                    if (gr.getTableName() == "u_ordem_compra" && state == "not requested") {
                        while (wf.next()) {
                            new Workflow().broadcastEvent(wf.sys_id, "oc.proxima.aprovacao");
                        }
                    }

                    if (gr.getTableName() == "u_solicitacao_compra" && state == "not requested") {
                        while (wf.next()) {
                            new Workflow().broadcastEvent(wf.sys_id, "sc.proxima.aprovacao");
                        }
                    }

                    if (gr.getTableName() == "u_item_da_solicitacao_compra" && state == "not requested") {
                        while (wf.next()) {
                            new Workflow().broadcastEvent(wf.sys_id, "sc.proxima.aprovacao");
                        }
                    }
    ...
    buscaOC: function (numero) {
        ...
        var query = gs.getMessage("u_numero_da_ordem={0}^u_sistema_de_origem=MV2000_WS", [numero]);
    ...
    buscaSC: function (numero) {
        ...
        var query = gs.getMessage("u_numero_da_solicitacao={0}^u_sistema_de_origem=MV2000_WS", [numero]);
