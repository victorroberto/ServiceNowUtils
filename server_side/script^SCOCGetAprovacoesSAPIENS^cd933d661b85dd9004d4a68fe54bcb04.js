...
SC_OC_GetAprovacoesSAPIENS.prototype = Object.extendsObject(AbstractAjaxProcessor, {

    /* Este é o template SOAP utilizado para buscar as aprovações no SAPIENS */
    SOAP_TEMPLATE: "<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/' xmlns:ser='http://services.senior.com.br'>" +
        "   <soapenv:Header/>" +
        "   <soapenv:Body>" +
        "      <ser:{0}>" +
        "         <user>ws_servicenow</user>" +
        "         <password>Sn@w2018</password>" +
    ...
    montaPayloadAprovacoes: function (user, obj_user, registroLog) {
        var parametros = "<codUsu>0</codUsu>" +
        ...
        var requestDataOC = gs.getMessage(this.SOAP_TEMPLATE, ["buscarOCPendentes", parametros]);
        var requestDataSC = gs.getMessage(this.SOAP_TEMPLATE, ["BuscarSCPendentes", parametros]);

        /* Chama a execução do integrador para as OC's do SAPIENS */
        this._executaIntegrador(obj_user, requestDataOC, 'function (integrador_gr) { ' +
            'new SC_OC_Sapiens_Sync().syncOc(integrador_gr, "' + registroLog + '");' +
            '}');

        /* Chama a execução do integrador para as SC's do SAPIENS */
        this._executaIntegrador(obj_user, requestDataSC, 'function (integrador_gr) { ' +
            'new SC_OC_Sapiens_Sync().syncSc(integrador_gr, "' + registroLog + '");' +
            '}');
    },

    /* Função reponsável por montar o payload do histórico de aprovações para o integrador do SAPIENS */
    montaPayloadHistorico: function (empresa, filial, numero, rotina, table, sys_id, callback) {
        var parametros = gs.getMessage(
            "<empresa>{0}</empresa>" +
            "<filial>{1}</filial>" +
            "<numero>{2}</numero>" +
            "<rotina>{3}</rotina>", [empresa, filial, numero, this.ROTINA_SAPIENS[rotina]]
        );
        ...
        var requestData = gs.getMessage(this.SOAP_TEMPLATE, ["historicoAprovacoes", parametros]);
    ...
    _executaIntegrador: function (obj_user, requestData, callback) {
        ...
        integrador.setDestinyEndpoint(env.g5_senior_services_sapiens.url + '/Synccom_unimedbh_servicenow');
