...
SC_OC_Sapiens_Sync.prototype = {
  ...
  getOcByExternalId: function (codigoEmpresa, codigoFilial, numero) {
    ...
    var query = gs.getMessage(
      "u_numero_da_ordem={0}^u_codigo_da_empresaSTARTSWITH{1} -^u_codigo_da_filialSTARTSWITH{2} -^u_sistema_de_origem=SAPIENS_WS",
      [numero, codigoEmpresa, codigoFilial]
    );
  ...
  getScByExternalId: function (codigoEmpresa, codigoFilial, numero) {
    ...
    var query = gs.getMessage(
      "u_numero_da_solicitacao={0}^u_codigo_da_empresaSTARTSWITH{1} -^u_codigo_da_filialSTARTSWITH{2} -^u_sistema_de_origem=SAPIENS_WS",
      [numero, codigoEmpresa, codigoFilial]
    );
    grOrdemCompra.addEncodedQuery(query);
    grOrdemCompra.query();
  ...
  syncApr: function (integrador_gr, registroLog) {
    var xmlString = XMLUtilJS.unescapeForXMLText(
      integrador_gr.getValue("u_data")
    );
    ...
    var h =
      obj["S:Body"]["ns2:historicoAprovacoesResponse"]["result"]["aprovacoes"];
    var sit =
      obj["S:Body"]["ns2:historicoAprovacoesResponse"]["result"]["situacao"];
    var eventName;
    ...
    if (gr.get(integrador_gr.getValue("u_table_sys_id"))) {
      ...
      for (var i = 0; i < h.length; i++) {
        ...
        new SC_OC_Sapiens_Sync().addApproverHistory(
          apr.aprovador,
          gr,
          sit_desc[apr.situacao],
          dt_hr,
          registroLog
        );
      }
  ...
  syncSc: function (integrador_gr, registroLog) {
    ...
    if (integrador_gr.getValue("u_data") == null) {
      ...
      this._insereLog(
        "atualizaWorklog",
        registroLog,
        "Solicitações de Compra - Sapiens: " + integrador_gr.getValue("u_data"),
        "",
        ""
      );
    }
    ...
    function persisteSc(grSolicitacaoCompra, sc) {
      grSolicitacaoCompra.setValue(
        "u_codigo_da_empresa",
        sc.codigoEmpresa + " - " + sc.nomeEmpresa
      );
      grSolicitacaoCompra.setValue(
        "u_codigo_da_filial",
        sc.codigoFilial + " - " + sc.nomeFilial
      );
      ...
      grSolicitacaoCompra.setValue(
        "u_valor_liquido",
        sc.valorUnitario * sc.quantidadeSolicitada
      );
    ...
    function addValues(grItem, item, order_id, numero) {
      ...
      grItem.setValue(
        "u_usuario_responsavel_pela_solicitacao",
      ...
      grItem.setValue(
        "u_valor_liquido",
        item.valorUnitario * item.quantidadeSolicitada
      );
      ...
      rateios.forEach(function (rateio) {
        OBSERVACAO_DO_RATEIO =
          "CC" +
          rateio.codigoCentroCustos +
          " - " +
          rateio.abreviaturaCentroCustos +
          ". " +
          "\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0" +
          (
            (parseFloat(rateio.valorRateio) * 100) /
            parseFloat(totalRateio)
          ).toFixed(2) +
          "% -> R$ " +
          rateio.valorRateio;
    ...
    var obj = helper.toObject();
    var scs =
      obj["S:Body"]["ns2:BuscarSCPendentesResponse"]["result"]["solicitacao"];
    ...
    for (var i = 0; i < scs.length; i++) {
      ...
      var grSolicitacaoCompra = this.getScByExternalId(
        sc.codigoEmpresa,
        sc.codigoFilial,
        sc.numero
      );
      ...
      if (grItem.sys_id) {
        ...
        gs.log(
          "Erro ao criar SC = " + JSON.stringify(sc),
          "SC_OC_Sapiens_Sync"
        );
      }
      ...
      var callback =
        ...
        'new SC_OC_Sapiens_Sync().syncApr(integrador_gr, "' +
        registroLog +
        '");' +
        "}";
      ...
      new SC_OC_GetAprovacoesSAPIENS().montaPayloadHistorico(
        sc.codigoEmpresa,
        sc.codigoFilial,
        numeroSequencia,
        "SC",
        grItem.getTableName(),
        grItem.getUniqueValue(),
        callback
      );
      ...
      this._insereLog(
        "atualizaWorklog",
        registroLog,
        "Solicitação " + sc.numero + " processada.",
        "",
        ""
      );
    }
    ...
    this._insereLog(
      "atualizaWorklog",
      registroLog,
      "Todas as solicitações do sapiens processadas",
      "",
      ""
    );
  },
  ...
  syncOc: function (integrador_gr, registroLog) {
    ...
    if (integrador_gr.getValue("u_data") == null) {
      ...
      this._insereLog(
        "atualizaWorklog",
        registroLog,
        "Ordens de Compra - Sapiens: " + integrador_gr.getValue("u_data"),
        "",
        ""
      );
    }
    ...
    function persisteOc(grOrdemCompra, oc) {
      grOrdemCompra.setValue(
        "u_codigo_da_empresa",
        oc.codigoEmpresa + " - " + oc.nomeEmpresa
      );
      grOrdemCompra.setValue(
        "u_codigo_da_filial",
        oc.codigoFilial + " - " + oc.nomeFilial
      );
      ...
      if (oc.totalAprovacao == oc.valorAproximado) {
        grOrdemCompra.setValue("u_valor_liquido", oc.valorAproximado);
      } else {
        if (oc.valorAproximado > 0) {
          if (oc.totalAprovacao < oc.valorAproximado) {
            grOrdemCompra.setValue("u_valor_liquido", oc.valorAproximado);
          } else if (oc.totalAprovacao > oc.valorAproximado) {
            grOrdemCompra.setValue("u_valor_liquido", oc.totalAprovacao);
          }
        } else if (oc.totalAprovacao > 0) {
          if (oc.totalAprovacao < oc.valorAproximado) {
            grOrdemCompra.setValue("u_valor_liquido", oc.valorAproximado);
          } else if (oc.totalAprovacao > oc.valorAproximado) {
            grOrdemCompra.setValue("u_valor_liquido", oc.totalAprovacao);
    ...
    var obj = helper.toObject();
    var ocs =
      obj["S:Body"]["ns2:buscarOCPendentesResponse"]["result"]["ordemCompra"];
    ...
    for (var i = 0; i < ocs.length; i++) {
      ...
      var grOrdemCompra = this.getOcByExternalId(
        oc.codigoEmpresa,
        oc.codigoFilial,
        oc.numero
      );
      ...
      if (order_id) {
        ...
        gs.log(
          "Erro ao criar OC = " + JSON.stringify(oc),
          "SC_OC_Sapiens_Sync"
        );
      }
    ...
    this._insereLog(
      "atualizaWorklog",
      registroLog,
      "Todas as ordens do sapiens processadas",
      "",
      ""
    );
  },
  ...
  addApprover: function (nextApprover, gr, state, dt_hr_aprovacao) {
    if (nextApprover != "NULL") {
      ...
      /* Verifica se o aprovador tem sysid */
      if (nextApproverSysId) {
        ...
        if (approval_check_gr) {
          ...
          if (state == "not requested") {
            ...
            if (gr.getTableName() == "u_ordem_compra") {
              while (wf.next()) {
                new Workflow().broadcastEvent(
                  wf.sys_id,
                  "oc.proxima.aprovacao"
                );
              }
            }
  ...
  getApprover: function (aprovador) {
    ...
    grAprov.addEncodedQuery(
      "group=f643c8096f4513001d86d7aabb3ee43e^user.user_name=" + aprovador
    );
    grAprov.query();
  ...
  /* Adicona uma nova aprovação de historico na tabela enviada por parâmetro */
  addApproverHistory: function (
    nextApprover,
    gr,
    state,
    dt_hr_aprovacao,
    ...
    this._insereLog(
      "atualizaWorklog",
      registroLog,
      "Adicionado historico de aprovação: " +
        nextApprover +
        ", " +
        state +
        ", " +
    ...
    /* Chama a função 'getApprover' e verifica se ela retorna um aprovador */
    var nextApproverSysId = this.getApproverHistory(nextApprover);

    /* Verifica se o aprovador tem sysid */
    if (nextApproverSysId) {
      var approval_check_gr = this.approverAlreadySetHistory(
        nextApproverSysId,
        gr
      );
      ...
      if (approval_check_gr) {
        ...
        /* Não existe um aprovador no chamado, cria nova aprovação */
        var approval_gr = new GlideRecord("sysapproval_approver");
        approval_gr.initialize();
        approval_gr.state = state || "Approved";
        approval_gr.source_table = gr.getTableName();
        approval_gr.approver = nextApproverSysId;
        approval_gr.u_data_aprovacao = dt_hr_aprovacao;
        approval_gr.sysapproval = gr.getUniqueValue();
        approval_gr.approval_column = "approval";
        approval_gr.comments = "Rotina de Sincronização SN";
        approval_gr.insert();
      ...
      apr_gr.approver = "admin";
      apr_gr.sysapproval = gr.getUniqueValue();
      apr_gr.u_data_aprovacao = dt_hr_aprovacao;
      apr_gr.approval_column = "approval";
      apr_gr.comments = "Este item foi aprovado por: " + nextApprover;
      apr_gr.insert();
    }
  },

  /* Verifica se usuário faz parte do grupo de aprovadores, se sim retorna seu sysid */
  getApproverHistory: function (aprovador) {
    var aprovadorFormatado = aprovador.slice(0, -10);
    var grAprov = new GlideRecord("sys_user_grmember");
    grAprov.addEncodedQuery(
      "group.sys_id=f643c8096f4513001d86d7aabb3ee43e^user.user_name=" +
        aprovador +
        "^ORuser.nameLIKE" +
        aprovadorFormatado
    );
  ...
  /* Verifica se já existe uma aprovação de historico ja criada */
  approverAlreadySetHistory: function (id, gr) {
    var approval_gr = new GlideRecord("sysapproval_approver");
    approval_gr.addEncodedQuery(
      "source_table=" +
        gr.getTableName() +
        "^sysapproval=" +
        gr.getUniqueValue() +
