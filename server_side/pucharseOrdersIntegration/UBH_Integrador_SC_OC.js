...
UBH_Integrador_SC_OC.prototype = Object.extendsObject(AbstractAjaxProcessor, {
  ...
  check: function (id) {
    ...
    if (id.substr(0, 3) == "mid") {
      ...
      while (gr.next()) {
        ...
        if (
          gr.getValue("state") == "processing" ||
          gr.getValue("state") == "ready"
        ) {
          state = "PROCESSING";
        }
  ...
  execute: function () {
    gs.log(
      "Entrou na UBH_Integrador_SC_OC.execute()",
      "SC_OC_PendingAproovals"
    );
    ...
    if (this.midServer) {
      ...
      if (this.destinyQueryParameter.length > 0) {
        r.setStringParameterNoEscape(
          "destinyEndpoint",
          this.destinyEndpoint + "?" + this.destinyQueryParameter
        );
      }
      ...
      try {
        ...
        sm.setStringParameter(
          "originAuth",
          "c2Vydl93c19zZXJ2aWNlbm93OiRUa25BT01idldAcQ=="
        );
  ...
  executeCallback: function (callback_function, obj) {
    gs.log(
      "Entrou na UBH_Integrador_SC_OC.executeCallback()",
      "SC_OC_PendingAproovals"
    );
    ...
    if (response) {
      ...
      grUI.setValue(
        "u_endpoint",
        response.destinyEndpoint || this.destinyEndpoint
      );
